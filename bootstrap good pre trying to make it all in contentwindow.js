...
function addDiv(doc) {
	...
	if (!(win in regedMuts)) {
	//if (!js.soundStormInited) {
		//js.soundStormMut = new win.MutationObserver(mutFunc);
		regedMuts[win] = new win.MutationObserver(mutFunc);
	...
	[].forEach.call(btns, function(btn, i) {
		...
		//js.soundStormMut.observe(btn, mutConfig);
		regedMuts[win].observe(btn, mutConfig);
	});
